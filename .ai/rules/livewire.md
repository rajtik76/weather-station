...
`Dashboard::stationReport()` reads the newest `station_reports` row for the selected sensor and prints it as the firmware sent it - firmware, reset reason, uptime, network, heap, buffered windows, failed uploads. It is dated by arrival (`created_at`), like the payload tail: the report describes the board at the moment it uploaded. Nothing here is derived on the server; if a figure looks wrong, the firmware sent it wrong. Absent until the first V2.1 upload, and the block renders nothing rather than dashes.
