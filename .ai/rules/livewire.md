...
## The station block is the board's word, not the server's

`Dashboard::stationReport()` reads the newest `station_reports` row for the selected sensor and prints it as the firmware sent it - firmware, reset reason, uptime, network role, heap, buffered windows, failed uploads. Never the SSID or the IP: the page is public, and they stay in `station_reports.data` for the operator only. It is dated by arrival (`created_at`), like the payload tail: the report describes the board at the moment it uploaded. Nothing here is derived on the server; if a figure looks wrong, the firmware sent it wrong. Absent until the first V2.1 upload, and the block renders nothing rather than dashes. The clock rows (`clockDrift()`) follow the same rule one level down: they show only when the report carries a measured step over a positive interval - a firmware before 2.2 sends none, and 2.2 sends zeros until its first re-sync after boot - and the three rows come and go together.
